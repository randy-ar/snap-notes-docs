...
class _ResponsePreviewPageState extends State<ResponsePreviewPage> {
  ...
  Widget build(BuildContext context) {
    final jsonResponse = const JsonEncoder.withIndent('  ').convert(widget.response);
    ...
    return Scaffold(
      ...
      child: Column(
        children: [
          ...
          // Content
          Expanded(
            child: _buildTabContent(_selectedTab, jsonResponse),
          ),
  ...
  Widget _buildTabContent(int index, String jsonResponse) {
    switch (index) {
      case 0:
        return _buildParsedDataTab();
      case 1:
  ...
  Widget _buildTreeTab() {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(16.0),
      child: _buildJsonTree(widget.response),
    );
  }
    ...
    return Card(
      ...
      child: Column(
          ...
          children: [
            ...
            const Gap(16),
            _buildDataRow('ID', widget.response['id']?.toString() ?? '-'),
            ...
            if (widget.response['gambarUrl'] != null)
              _buildDataRow('Image URL', widget.response['gambarUrl'].toString()),
            if (widget.response['sudahDikonfirmasi'] != null)
              _buildDataRow('Dikonfirmasi', widget.response['sudahDikonfirmasi'].toString()),
            const Gap(16),
