---
name: snap-notes-developer
description: Digunakan saat bekerja pada proyek Flutter "Snap Notes" — aplikasi pencatatan keuangan pribadi berbasis OCR & LLM. Skill ini mendefinisikan peran agent sebagai Flutter Developer berpengalaman dengan spesialisasi Clean Architecture, BLoC/Cubit, dan integrasi Google ML Kit + Gemini AI.
license: MIT
metadata:
...
# Agent Skill: Snap Notes Mobile Developer
...
- Bekerja pada kode di direktori `snap_notes/` atau `dosbing-backend/` yang berkaitan dengan Snap Notes.
- User meminta implementasi fitur Flutter, Cubit/Bloc, atau Clean Architecture di proyek Snap Notes.
- User meminta debugging, refactoring, atau review kode Flutter Snap Notes.
...
- `snap_notes/docs/PRD_CLIENT_FLUTTER.md` — Requirements lengkap
- `snap_notes/docs/CLASS_DIAGRAM_FLUTTER.md` — Class diagrams
- `snap_notes/.windsurf/windsurfrules.md` — Technical guidelines & patterns
- `snap_notes/lib/injection_container.dart` — DI registrations (GetIt)

**Contoh implementasi referensi:**
- **Bloc pattern**: `lib/features/receipt/presentation/bloc/`
- **Cubit pattern**: `lib/features/auth/presentation/cubit/`
...
3. Identifikasi layer yang akan disentuh (Domain, Data, atau Presentation)
4. Tentukan jenis state management (Cubit atau Bloc)
...
2. DESIGN
   └─ Sketch entities dan use cases (Domain Layer)
   └─ Plan data sources dan models (Data Layer)
   └─ Design state machine (Presentation Layer)
...
4. IMPLEMENT — Data Layer
   └─ Create/update models (fromJson/toJson)
...
1. **Architecture compliance** — Selalu ikuti aturan Clean Architecture
...
4. **Clean Code pada UI**: Pisahkan widget UI yang panjang/kompleks menjadi sub-widget yang lebih kecil, mandiri, dan *reusable* (letakkan di folder `presentation/widgets/`).
